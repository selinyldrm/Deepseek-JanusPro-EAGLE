...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # [SY]: comment the below 3 lines out for llamagen with eagle3 training.
        ...
        item = {
            ...
            "hidden_states": hidden_states, # [SY]: comment out for llamagen with eagle3 training.
...
class DataCollatorWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # batch_input_ids = torch.cat([item['input_ids'] for item in features]) # [SY] Eagle3: remove padding since cond_idx introduced
        # batch_cond_idx = torch.cat([item['cond_idx'].to(batch_input_ids.device) for item in features]) # [SY] Eagle3
        batch_input_ids = torch.cat([self.paddingtensor2D(item['input_ids'], max_length) for item in features])
        batch_hidden_states = torch.cat([self.paddingtensor(item['hidden_states'], max_length) for item in features]) # [SY] Eagle3: comment this out
        batch_target = torch.cat([self.paddingtensor(item['target'], max_length) for item in features]) # [SY] Eagle3: comment this out
        ...
        batch = {
            ...
            "hidden_states": batch_hidden_states, # [SY] Eagle3: comment this out
...
class CoupledDataset(Dataset):
    ...
    def prepare_data(self, data, conditioned=True):
        ...
        if not conditioned:
            ...
            # print("hidden_states_zeropadding shape: ", hidden_states_zeropadding.shape)
            data['uncond_hidden_states'] = torch.cat([hidden_states_zeropadding, data['uncond_hidden_states']], dim=0)
            # print("new 'uncond_hidden_states' shape: ", data['uncond_hidden_states'].shape)

        # print("conditioned: ", conditioned)
        # print("data input_ids: ", data[f"{prefix}_input_ids"].shape)
        # if conditioned:
            # print("original hidden_states: ", data[f"{prefix}_hidden_states"].shape)
        
        input_ids = data[f"{prefix}_input_ids"][:self.max_len][None, :]
        hidden_states = data[f"{prefix}_hidden_states"][:self.max_len][None, :]
        # print("truncated hidden_states shape: ", hidden_states.shape)
        ...
        # input_ids_targets
        zeropadding = torch.tensor([[0]])
        # print("data prepare input_ids: ", input_ids.shape)
        ...
        # print("data prepare hidden_states: ", hidden_states.shape)
        target = hidden_states[:, 1:, :]
        zeropadding = torch.zeros(1, 1, target.shape[2])
        target = torch.cat((target, zeropadding), dim=1)
