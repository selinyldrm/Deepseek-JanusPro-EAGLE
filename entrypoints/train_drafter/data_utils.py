...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.model == "lumina_mgpt" or self.model == "anole":
            ...
            # target_hidden: [576, 4096] 
            # input_embeds:  [576, 4096]
            
            # We return them exactly as saved. 
...
class DataCollatorWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        is_janus = "target_hidden" in features[0]

        if is_janus:
            # 1. Determine Max Length (Usually 576, but safe to calculate)
            max_len = max(item['target_hidden'].shape[1] for item in features)
            
            # 2. Pad and Concatenate
            # We treat 'target_hidden' as the primary sequence
            batch_target_hidden = torch.cat([
                self.paddingtensor(item['target_hidden'], max_len) for item in features
            ])
            
            # We treat 'input_embeds' the same way
